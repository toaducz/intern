...
class CompanyRepository extends BaseUnitOfWork implements ICompanyRepository{ constructor(){ super(); }
  ...
  async configSlot(companyId: string, queryData: any, session: ClientSession, slotTime: number, slot: any): Promise<any> {
    try {
      const _id = new mongoose.Types.ObjectId(companyId);
      const query: any = {
        _id: _id,
        isDeleted: queryData.isDeleted,
        isActive: queryData.isActive
      ...
      const company: any = await CompanyWithBase.findOneAndUpdate(query, updateData, { session });
      return company;
    }
    catch (error: any) {
        throw new Error(`Error occured at configSlot in CompanyRepository: ${error.message}`);
    }
  }
}
